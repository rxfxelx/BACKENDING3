...
CONSENT_BUTTONS = [
    "button#L2AGLb","button:has-text('Aceitar tudo')","button:has-text('Concordo')",
    "button:has-text('Aceitar')","button:has-text('I agree')","button:has-text('Accept all')",
]
UA_POOL = [
...
async def _safe_unroute_close(context, page, handler):
    try:
        if context:
            try: await context.unroute("**/*", handler)
        ...
        for attempt in range(3):
            ...
            async def _route_handler(route):
                try:
                    ...
                    # se o contexto/página fechar no meio, ignoramos
                    pass
            ...
            try:
                ...
                for local in locais:
                    ...
                    for term in terms:
                        ...
                        while True:
                            ...
                            if page.is_closed(): raise TargetClosedError("page closed")

                            start = idx * 20
                            q = term
                            if captcha_hits_term > 0:
                                q = (term + random.choice([""," ","  "," ★"," ✔"," ✓"])).strip()
                ...
                await _safe_unroute_close(context, page, _route_handler)
                await asyncio.sleep(0.2)
                await _safe_stop()   # força recriar browser
                if attempt >= 2:
                    return
                continue
            except Exception:
