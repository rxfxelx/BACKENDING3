import os, random, urllib.parse, base64, unicodedata
from typing import AsyncGenerator, List, Set, Optional
from asyncio import CancelledError, InvalidStateError

from playwright.async_api import async_playwright, TimeoutError as PWTimeoutError, Browser, Page
...
RESULT_CONTAINERS = [".rlfl__tls",".VkpGBb",".rllt__details",".rllt__wrapped","div[role='article']","#search","div[role='main']"]
CONSENT_BUTTONS = ["button#L2AGLb","button:has-text('Aceitar tudo')","button:has-text('Concordo')","button:has-text('Aceitar')","button:has-text('I agree')","button:has-text('Accept all')"]
UA_POOL = [
...
async def _ensure_browser(restart: bool = False) -> Browser:
    global _playwright, _browser
    if restart and _browser:
    ...
    name = os.getenv("BROWSER", getattr(settings, "BROWSER", "chromium"))
    headless = getattr(settings, "HEADLESS", True)
    _browser = await getattr(_playwright, name).launch(
        headless=headless,
...
async def _try_accept_consent(page: Page) -> None:
    try:
...
async def _extract_phones_from_page(page: Page) -> List[str]:
    phones: Set[str] = set()
    try:
        ...
        for sel in RESULT_CONTAINERS:
            try:
                ...
                for block in blocks or []:
                    for n in extract_phones_from_text(block): phones.add(n)
            except Exception:
                continue
    except Exception:
        pass
...
async def _is_captcha_or_sorry(page: Page) -> bool:
    try:
        txt = (await page.content())[:100000].lower()
        if "/sorry/" in txt or "unusual traffic" in txt or "recaptcha" in txt or "g-recaptcha" in txt: return True
...
async def search_numbers(
    ...
    *,
    ...
    ua = getattr(settings, "USER_AGENT", None) or random.choice(UA_POOL)
    ...
    async def _route_handler(route):
        ...
        if rtype in {"image", "media", "font", "stylesheet"}: await route.abort()
        ...
        for local in locais:
            ...
            for term in terms:
                ...
                while True:
                    ...
                    start = idx * 20
                    q = (term + (random.choice([""," ","  "," ★"," ✔"," ✓"]) if captcha_hits_term > 0 else "")).strip()
                    ...
                    for ph in phones:
                        if ph not in seen:
                            seen.add(ph); new += 1; total_yield += 1
                            yield ph
                            if target and total_yield >= target: return

                    empty_pages = empty_pages + 1 if new == 0 else 0
                    if empty_pages >= empty_limit: break
                    await page.wait_for_timeout(180 + min(1200, int(idx * 35 + random.randint(80, 180))))
                    idx += 1
    except InvalidStateError:
        await _ensure_browser(restart=True)
        return
